...
export async function getCompanySettings(): Promise<CompanySettings> {
  ...
  return {
    targetMargin: parseFloat(
      settingsMap[SETTING_KEYS.TARGET_MARGIN] ||
        String(DEFAULT_SETTINGS.targetMargin),
    ),
    hoursPerMonth: parseFloat(
      settingsMap[SETTING_KEYS.HOURS_PER_MONTH] ||
        String(DEFAULT_SETTINGS.hoursPerMonth),
    ),
    targetUtilization: parseFloat(
      settingsMap[SETTING_KEYS.TARGET_UTILIZATION] ||
        String(DEFAULT_SETTINGS.targetUtilization),
    ),
    costPerPersonPerMonth: parseFloat(
      settingsMap[SETTING_KEYS.COST_PER_PERSON_PER_MONTH] ||
        String(DEFAULT_SETTINGS.costPerPersonPerMonth),
    ),
    overheadPeople: parseInt(
      settingsMap[SETTING_KEYS.OVERHEAD_PEOPLE] ||
        String(DEFAULT_SETTINGS.overheadPeople),
      10,
    ),
  };
}
