...
export async function calculateOverheadAllocation(
  ...
  const peopleOverheadAnnual =
    companySettings.overheadPeople * companySettings.costPerPersonPerMonth * 12;
  ...
  if (
    ...
    totalBillableHC._sum.billableHeadcount === 0
  ) {
    return 0;
  }
 ...
 */
export async function calculateAnnualMetrics(departmentId: string) {
  const department = await prisma.department.findUnique({
    where: { id: departmentId },
  });
...
export async function getDepartmentMonthlyResult(
  ...
  return await prisma.result.upsert({
    ...
    create: {
      ...
      revenueFromHours:
        revenueFromHours > 0 ? new Decimal(revenueFromHours) : null,
      totalRevenue: new Decimal(safeTotalRevenue), // Sempre salvar, mesmo se 0
      objective:
        objectiveValue && objectiveValue > 0
          ? new Decimal(objectiveValue)
          : null,
      performance:
        performance !== null && performance >= 0
          ? new Decimal(performance)
          : null,
      utilizationRate:
        utilizationRate !== null && utilizationRate >= 0
          ? new Decimal(utilizationRate)
          : null,
      calculatedBy: "system",
    },
    update: {
