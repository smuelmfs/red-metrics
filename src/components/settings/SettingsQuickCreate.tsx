...
export default function SettingsQuickCreate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const settings = lines.map((line) => {
        ...
        if (!rest) {
          throw new Error(
            `Linha inválida (esperado formato key=value|descrição): "${line}"`,
          );
        }
  ...
  return (
    <form
      ...
      className="bg-white rounded-lg shadow-md p-4 lg:p-6 space-y-4"
    >
      <div>
        <p className="text-sm text-gray-700 mb-2">
          Não existem configurações globais ainda. Cole ou edite as linhas
          abaixo no formato{" "}
        ...
        <p className="text-xs text-gray-500 mb-3">
          Exemplos já estão preenchidos: targetMargin, hoursPerMonth,
          targetUtilization, costPerPersonPerMonth, overheadPeople.
        </p>
        <textarea
          rows={8}
