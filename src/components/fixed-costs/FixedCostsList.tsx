...
export default function FixedCostsList({ fixedCosts }: FixedCostsListProps) {
  ...
  const groupedByCategory = fixedCosts.reduce(
    (acc, cost) => {
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {costs.map((cost) => (
                    <tr
                      key={cost.id}
                      className={!cost.isActive ? "opacity-50" : ""}
                    >
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900">
                        €
                        {cost.monthlyAmount.toLocaleString("pt-BR", {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-900">
                        €
                        {(cost.monthlyAmount * 12).toLocaleString("pt-BR", {
                          minimumFractionDigits: 2,
                        })}
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-600">
                        ...
                        {cost.endDate &&
                          ` - ${new Date(cost.endDate).toLocaleDateString("pt-BR")}`}
