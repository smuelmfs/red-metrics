...
export default function RetainerCatalogList({
  catalog,
  departments,
}: RetainerCatalogListProps) {
    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow-sm px-4 py-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <p className="text-xs sm:text-sm text-gray-600">
          Campos de custo e margem só aparecem quando o{" "}
          <span className="font-semibold">Custo Interno/h</span> está
          configurado. Quando o valor não foi definido para um item, a tabela
          mostra <span className="font-mono">-</span> (não configurado).
        </p>
        ...
        <div key={department.id} className="bg-white rounded-lg shadow-md p-6">
          ...
          {items.length === 0 ? (
            <p className="text-gray-500 text-sm">
              Nenhum item no catálogo para este departamento. Crie um item para
              começar.
            </p>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {items.map((item) => {
                    const margin = item.monthlyMargin
                      ? Number(item.monthlyMargin)
                      : null;
                    const marginPct = item.marginPercentage
                      ? Number(item.marginPercentage)
                      : null;
                    ...
                    return (
                      <tr key={item.id}>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900">
                          €
                          {Number(item.monthlyPrice).toLocaleString("pt-BR", {
