...
const ENCRYPTION_KEY =
  process.env.ODOO_ENCRYPTION_KEY || "default-key-change-in-production";
...
export async function syncOdooHours(
  ...
  try {
    ...
    console.log(
      `[Odoo Sync] Buscando horas para ${month}/${year} com filtros:`,
      billingTypes || "todos",
    );
    ...
    console.log(
      `[Odoo Sync] Dados recebidos do Odoo: ${hoursData.length} departamento(s) com horas registradas`,
    );

    if (hoursData.length === 0) {
      console.log(
        `[Odoo Sync] Nenhum dado encontrado no Odoo para ${month}/${year}`,
      );
    }
    ...
    for (const hours of hoursData) {
      try {
        ...
        if (!department) {
          ...
          let counter = 1;
          while (
            await prisma.department.findUnique({ where: { code: finalCode } })
          ) {
          ...
          try {
            ...
            console.log(
              `[Odoo Sync] Departamento criado: ${hours.departmentName} (código: ${finalCode})`,
            );
        ...
        syncedCount++;
        console.log(
          `[Odoo Sync] Sincronizado: ${hours.departmentName} - ${hours.totalHours}h (${month}/${year})`,
        );
      } catch (error: any) {
    ...
    console.log(
      `[Odoo Sync] Total sincronizado: ${syncedCount} departamento(s)`,
    );
