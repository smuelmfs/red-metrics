...
export class OdooClient {
  ...
  private normalizeUrl(url: string): {
    ...
    try {
      ...
      throw new Error(
        `URL inválida: ${url}. Use o formato: https://odoo.example.com`,
      );
    }
  }
        ...
        client.methodCall(
          ...
          (error: any, uid: number) => {
            if (error) {
              ...
              if (
                errorMsg.includes("unknown xml-rpc tag") ||
...
- A URL não precisa de barra final`;
            ...
            if (!uid) {
              reject(
                new Error(
                  "Credenciais inválidas. Verifique usuário, senha e nome do banco de dados.",
                ),
              );
              return;
            }
        ...
        reject(
          new Error(`Erro ao criar cliente XML-RPC: ${error.message || error}`),
        );
      }
    });
  }
      ...
      models.methodCall(
        ...
        (error: any, departments: any[]) => {
          if (error) {
            reject(
              new Error(
                `Erro ao buscar departamentos: ${error.message || error}`,
              ),
            );
            return;
          }
  ...
  async getAllDepartmentsHours(
    ...
    if (year < 2026) {
      throw new Error(
        `Apenas anos a partir de 2026 são permitidos. Ano solicitado: ${year}`,
      );
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      models.methodCall(
        ...
        (error: any, result: any) => {
          if (error) {
            reject(
              new Error(`Erro ao buscar horas: ${error.message || error}`),
            );
            return;
          }

          console.log(
            `[Odoo Client] Resultado do read_group:`,
          ...
          console.log(
            `[Odoo Client] Filtros aplicados:`,
            JSON.stringify(filters, null, 2),
          );
          console.log(
            `[Odoo Client] Tipos de faturamento solicitados:`,
            billingTypes,
          );

          // Log detalhado de cada grupo recebido
          console.log(
            `[Odoo Client] Grupos recebidos do Odoo:`,
            groups.map((g: any) => ({
          ...
          for (const group of groups) {
            ...
            if (
              !group.department_id ||
              group.department_id === false ||
              group.department_id === null
            ) {
              console.log(
                `[Odoo Client] Ignorando grupo sem departamento:`,
                group,
              );
              continue;
            }
            ...
            const deptId = Array.isArray(group.department_id)
              ? group.department_id[0]
              : group.department_id;
            const deptName = Array.isArray(group.department_id)
            ...
            if (typeof group.unit_amount === "number") {
              ...
              group.unit_amount &&
              typeof group.unit_amount === "object"
            ) {
            ...
            console.log(
              `[Odoo Client] Departamento: ${deptName} (ID: ${deptId}) - ${totalHours}h (count: ${group.department_id_count || "N/A"})`,
            );
            ...
            if (totalHours > 0) {
              hoursData.push({
                ...
                billingType:
                  billingTypes && billingTypes.length === 1
                    ? billingTypes[0]
                    : undefined,
              });
            } else {
              console.log(
                `[Odoo Client] Ignorando departamento ${deptName} (ID: ${deptId}) - sem horas no período`,
              );
            }
          }
          ...
          console.log(
            `[Odoo Client] Total de departamentos com horas: ${hoursData.length}`,
          );
          resolve(hoursData);
        },
      );
