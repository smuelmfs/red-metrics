...
export default async function DepartmentDetailPage({
  ...
  try {
    monthlyResult = await getDepartmentMonthlyResult(
      params.id,
      selectedMonth,
      selectedYear,
    );
  ...
  const performance = monthlyResult?.performance
    ? Number(monthlyResult.performance)
    : null;
  const status =
    performance !== null
      ? performance >= 100
  ...
  const directCost = department.directCostAnnual
    ? Number(department.directCostAnnual)
    : 0;
  const overhead = department.overheadAllocatedAnnual
    ? Number(department.overheadAllocatedAnnual)
    : 0;
  ...
  return (
    <div className="w-full max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-6">
        <div className="bg-white rounded-lg shadow-md p-4 lg:p-6">
          ...
          <p className="text-2xl lg:text-3xl font-bold text-gray-900">
            €
            {monthlyResult
              ? Number(monthlyResult.totalRevenue).toLocaleString("pt-BR", {
        ...
        <div className="bg-white rounded-lg shadow-md p-4 lg:p-6">
          ...
          <p className="text-2xl lg:text-3xl font-bold text-gray-900">
            €
            {objective
              ? Number(objective.targetValue).toLocaleString("pt-BR", {
                  minimumFractionDigits: 2,
                })
              : "N/A"}
          </p>
          ...
          <p
            className={`text-2xl lg:text-3xl font-bold ${
              monthlyResult && objective
                ? Number(monthlyResult.totalRevenue) -
        ...
        <div className="bg-white rounded-lg shadow-md p-4 lg:p-6">
          ...
          <p
            className={`text-2xl lg:text-3xl font-bold ${getStatusTextClasses(status)}`}
          >
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900">
                ...
                {monthlyResult?.revenueFromHours
                  ? Number(monthlyResult.revenueFromHours).toLocaleString(
                      "pt-BR",
                      { minimumFractionDigits: 2 },
                    )
                  : "0,00"}
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900">
                ...
                €
                {activeRetainers
                  .reduce((sum, r) => sum + Number(r.monthlyRevenue), 0)
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900">
                ...
                {plannedHours?.projectRevenue
                  ? Number(plannedHours.projectRevenue).toLocaleString(
                      "pt-BR",
                      { minimumFractionDigits: 2 },
                    )
                  : "0,00"}
            ...
            <div>
              ...
              <p className="text-lg font-semibold text-gray-900">
                ...
                {department.overheadAllocatedAnnual
                  ? Number(department.overheadAllocatedAnnual).toLocaleString(
                      "pt-BR",
                      { minimumFractionDigits: 2 },
                    )
                  : "N/A"}
            ...
            <div>
              <p className="text-sm text-gray-600 mb-1">
                Receita Mínima Sustentável (Anual)
              </p>
              <p className="text-lg font-semibold text-gray-900">
                €
                {department.minimumRevenueAnnual
                  ? Number(department.minimumRevenueAnnual).toLocaleString(
                      "pt-BR",
                      { minimumFractionDigits: 2 },
                    )
                  : "N/A"}
      ...
      <div className="mt-6 bg-white rounded-lg shadow-md p-6">
        <h2 className="text-lg lg:text-xl font-semibold text-gray-900 mb-4">
          Composição do Objetivo Mínimo Anual
        </h2>
        <div className="space-y-3">
          ...
          <div className="flex justify-between items-center py-2 border-b border-gray-200">
            ...
            <span className="text-sm font-semibold text-gray-900">
              €
              {overhead > 0
                ? overhead.toLocaleString("pt-BR", { minimumFractionDigits: 2 })
                : "N/A"}
            </span>
          </div>
          ...
          <div className="flex justify-between items-center py-2 bg-red-50 rounded-lg px-4">
            <span className="text-sm font-bold text-gray-900">
              Receita Mínima Anual Necessária
            </span>
          ...
          <p className="text-xs text-gray-500 mt-2">
            Fórmula: (Custo Direto + Overhead) / (1 - Margem Alvo) = Receita
            Mínima
          </p>
        </div>
      </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {activeRetainers.map((retainer) => (
                  <tr key={retainer.id}>
                    ...
                    <td className="px-4 py-3 text-sm text-gray-900">
                      €
                      {Number(retainer.monthlyPrice).toLocaleString("pt-BR", {
                        minimumFractionDigits: 2,
                      })}
                    </td>
                    <td className="px-4 py-3 text-sm text-gray-900">
                    ...
                    <td className="px-4 py-3 text-sm font-medium text-gray-900">
                      €
                      {Number(retainer.monthlyRevenue).toLocaleString("pt-BR", {
                        minimumFractionDigits: 2,
                      })}
                    </td>
                  </tr>
