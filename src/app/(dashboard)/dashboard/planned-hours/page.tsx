...
export default async function PlannedHoursPage({
  ...
  const serializedDepartments = departments.map((dept) => ({
    ...
    costPerPersonPerMonth: dept.costPerPersonPerMonth
      ? Number(dept.costPerPersonPerMonth)
      : null,
    targetUtilization: Number(dept.targetUtilization),
    averageHourlyRate: Number(dept.averageHourlyRate),
    directCostAnnual: dept.directCostAnnual
      ? Number(dept.directCostAnnual)
      : null,
    billableHoursAnnual: dept.billableHoursAnnual
      ? Number(dept.billableHoursAnnual)
      : null,
    revenueCapacityAnnual: dept.revenueCapacityAnnual
      ? Number(dept.revenueCapacityAnnual)
      : null,
    overheadAllocatedAnnual: dept.overheadAllocatedAnnual
      ? Number(dept.overheadAllocatedAnnual)
      : null,
    minimumRevenueAnnual: dept.minimumRevenueAnnual
      ? Number(dept.minimumRevenueAnnual)
      : null,
  ...
  const serializedPlannedHours = plannedHours.map((ph) => ({
    ...
    targetHoursPerMonth: ph.targetHoursPerMonth
      ? Number(ph.targetHoursPerMonth)
      : null,
    targetUtilization: ph.targetUtilization
      ? Number(ph.targetUtilization)
      : null,
    targetAvailableHours: ph.targetAvailableHours
      ? Number(ph.targetAvailableHours)
      : null,
    actualBillableHours: ph.actualBillableHours
      ? Number(ph.actualBillableHours)
      : null,
    actualUtilization: ph.actualUtilization
      ? Number(ph.actualUtilization)
      : null,
        ...
        {departments.length === 0 ? (
          ...
          <div className="space-y-4">
            {serializedDepartments.map((dept) => {
              const planned = serializedPlannedHours.find(
                (ph) => ph.departmentId === dept.id,
              );
              // Se a integração Odoo estiver ativa, todos os departamentos podem ter horas sincronizadas
              const hasOdooSync =
                odooIntegration?.isEnabled && planned?.syncedFromOdoo;
              return (
                <PlannedHoursForm
                  key={dept.id}
                  department={dept}
