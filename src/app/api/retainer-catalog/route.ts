...
export async function POST(request: NextRequest) {
  try {
    ...
    if (validatedData.internalHourlyCost && validatedData.hoursPerMonth) {
      monthlyCost =
        validatedData.internalHourlyCost * validatedData.hoursPerMonth;
    ...
    const catalogItem = await prisma.retainerCatalog.create({
      data: {
        ...
        marginPercentage: marginPercentage
          ? new Decimal(marginPercentage)
          : null,
        baseHours: validatedData.baseHours
          ? new Decimal(validatedData.baseHours)
          : null,
        basePrice: validatedData.basePrice
          ? new Decimal(validatedData.basePrice)
          : null,
      },
      include: {
        department: true,
