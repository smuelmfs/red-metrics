...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(
      `[Odoo Synced Data] Encontrados ${syncedHours.length} registros com syncedFromOdoo=true`,
    );
    ...
    const stats = {
      ...
      totalHours: syncedHours.reduce(
        (sum, ph) =>
          sum + (ph.actualBillableHours ? Number(ph.actualBillableHours) : 0),
        0,
      ),
      lastSyncDate:
        syncedHours.length > 0
          ? syncedHours
              .reduce(
                (latest, ph) => {
